...
class TrainingLoop:
    ...
    async def edit_phase(self, test_video: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            return {"success": result.get("success", False), "phase": "edit", "result": result}
        except Exception as e:
    ...
    async def evaluate_phase(self, video_path: Optional[str] = None) -> Dict:
        ...
        if not video_path:
            ...
            test_edits = sorted(self.renders_dir.glob("test_edit_*.mp4"), key=lambda p: p.stat().st_mtime)
            if not test_edits:
        ...
        try:
            ...
            return {"success": result.get("success", False), "phase": "evaluate", "result": result}
        except Exception as e:
    ...
    async def improve_phase(self) -> Dict:
        ...
        try:
            ...
            return {"success": result.get("success", False), "phase": "improve", "result": result}
        except Exception as e:
    ...
    async def run_continuous(self, test_video: Optional[str] = None):
        ...
        print(f"🔄 Starting continuous training loop (interval: {self.train_interval_hours} hours)")
...
def main():
    ...
    parser.add_argument("--test-video", type=str, help="Path to test video for editing phase")
    parser.add_argument("--config", type=str, default="core/config.json", help="Path to config file")
