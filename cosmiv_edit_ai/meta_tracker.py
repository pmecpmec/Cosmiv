...
class MetaTracker:
    ...
    def analyze_patterns(self, videos: List[Dict]) -> Dict:
        ...
        if "speed" in text_lower or "ramp" in text_lower:
            patterns["speed_ramps"] = min(100, (text_lower.count("speed") + text_lower.count("ramp")) * 10)
        
        if "subtitle" in text_lower or "caption" in text_lower or "text" in text_lower:
            patterns["subtitles"] = min(100, (text_lower.count("subtitle") + text_lower.count("caption")) * 10)
        ...
        patterns["common_keywords"] = sorted(word_freq.items(), key=lambda x: x[1], reverse=True)[:10]
    ...
    def update_trends(self, platform: str, new_patterns: Dict):
        ...
        if not self.trends_file.exists():
            trends_data = {
                ...
                "last_updated": None,
                "trends": {
                    "youtube_shorts": {"current_meta": [], "trending_patterns": [], "evolution_history": []},
                    "tiktok": {"current_meta": [], "trending_patterns": [], "evolution_history": []},
                    "instagram_reels": {"current_meta": [], "trending_patterns": [], "evolution_history": []}
                },
        ...
        trends_data["trends"][platform]["current_meta"] = [
            entry for entry in trends_data["trends"][platform]["current_meta"]
        ...
        if trends_data["trends"][platform]["current_meta"]:
            ...
            aggregated = {
                "beat_cuts": sum(p["patterns"].get("beat_cuts", 0) for p in recent_patterns) / len(recent_patterns),
                "speed_ramps": sum(p["patterns"].get("speed_ramps", 0) for p in recent_patterns) / len(recent_patterns),
                "subtitles": sum(p["patterns"].get("subtitles", 0) for p in recent_patterns) / len(recent_patterns),
                "zooms": sum(p["patterns"].get("zooms", 0) for p in recent_patterns) / len(recent_patterns),
            }
        ...
        trends_data["trends"][platform]["evolution_history"].append({
            "timestamp": timestamp,
            "snapshot": aggregated if trends_data["trends"][platform]["current_meta"] else {}
        })
        
        # Keep evolution history limited
        if len(trends_data["trends"][platform]["evolution_history"]) > 100:
            trends_data["trends"][platform]["evolution_history"] = trends_data["trends"][platform]["evolution_history"][-100:]
    ...
    def fetch_and_update(self) -> Dict:
        ...
        if trend_sources.get("instagram_reels", False):
            print("  Instagram Reels tracking not yet implemented (requires API access)")
