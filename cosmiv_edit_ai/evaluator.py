...
class Evaluator:
    ...
    def __init__(self, config_path: str = "core/config.json"):
        ...
        self.metrics_weights = self.config.get("evaluation_metrics", {
    ...
    def _generate_suggestions(self, metrics: Dict, overall_score: float) -> List[str]:
        ...
        if metrics.get("pacing_consistency", 0) < 70:
            suggestions.append("Improve pacing consistency - ensure cuts are evenly timed")
        
        if metrics.get("beat_alignment", 0) < 70:
            suggestions.append("Better align cuts with audio beats for more dynamic feel")
        ...
        if overall_score < 70:
            suggestions.append("Overall: Focus on improving lowest-scoring metrics first")
    ...
    def _save_feedback(self, evaluation: Dict):
        ...
        if len(feedback_data["performance_history"]) > 1000:
            feedback_data["performance_history"] = feedback_data["performance_history"][-1000:]
