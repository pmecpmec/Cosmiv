...
async def get_user(
    ...
    with get_session() as session:
        ...
        entitlements = session.exec(select(Entitlement).where(Entitlement.user_id == user_id)).all()
...
async def get_queue_status(
    ...
    with get_session() as session:
        pending = session.exec(
            select(Job).where(Job.status == JobStatus.PENDING).order_by(Job.created_at.desc()).limit(50)
        ).all()

        processing = session.exec(
            select(Job).where(Job.status == JobStatus.PROCESSING).order_by(Job.created_at.desc()).limit(50)
        ).all()

        # Get stats
        total_pending = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.PENDING)).one()

        total_processing = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.PROCESSING)).one()

        # Recent failures
        recent_failures = session.exec(
            select(Job).where(Job.status == JobStatus.FAILED).order_by(Job.created_at.desc()).limit(10)
        ).all()
...
async def get_analytics(
    ...
    with get_session() as session:
        ...
        active_users = session.exec(select(func.count(User.id)).where(User.is_active == True)).one()  # noqa: E712
        ...
        success_jobs = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.SUCCESS)).one()
        failed_jobs = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.FAILED)).one()
        pending_jobs = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.PENDING)).one()
        processing_jobs = session.exec(select(func.count(Job.id)).where(Job.status == JobStatus.PROCESSING)).one()

        # Payment stats (from entitlements)
        free_users = session.exec(select(func.count(Entitlement.id)).where(Entitlement.plan == "free")).one()
        pro_users = session.exec(select(func.count(Entitlement.id)).where(Entitlement.plan == "pro")).one()

        # Recent activity (last 24 hours)
        yesterday = datetime.utcnow() - timedelta(days=1)
        recent_jobs = session.exec(select(func.count(Job.id)).where(Job.created_at >= yesterday)).one()

        recent_users = session.exec(select(func.count(User.id)).where(User.created_at >= yesterday)).one()
...
async def list_all_jobs(
    ...
    with get_session() as session:
        ...
        jobs = session.exec(query.order_by(Job.created_at.desc()).offset(offset).limit(limit)).all()
