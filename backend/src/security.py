...
async def verify_token(authorization: str = Header(None, alias="Authorization")) -> str:
    ...
    try:
        ...
        if not user_id:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid token payload: missing user identifier"
            )
        return user_id
...
def validate_file_path(path: str, allowed_dir: str) -> str:
    ...
    if not abs_path.startswith(abs_allowed):
        security_logger.warning(f"Path traversal attempt detected: {path} outside {allowed_dir}")
...
def validate_user_owns_resource(user_id: str, resource_user_id: str) -> None:
    ...
    if user_id != resource_user_id:
        log_security_event("UNAUTHORIZED_ACCESS_ATTEMPT", user_id, {"attempted_resource_user": resource_user_id})
        raise HTTPException(
