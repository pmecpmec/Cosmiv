...
async def get_current_admin_user(current_user: User = Depends(get_current_user)) -> User:
    ...
    if hasattr(current_user, "is_admin") and not current_user.is_admin:
        if hasattr(current_user, "role") and current_user.role not in ["ADMIN", "OWNER"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Not enough permissions - admin access required"
            )
    return current_user
...
async def register(user_data: UserRegister):
    ...
    with get_session() as session:
        ...
        if existing:
            log_security_event("REGISTRATION_ATTEMPT_DUPLICATE", None, {"email": user_data.email})
...
async def login(credentials: UserLogin):
    ...
    with get_session() as session:
        ...
        if not user:
            log_security_event("LOGIN_FAILED_USER_NOT_FOUND", None, {"email": credentials.email})
...
async def create_dev_token(user_id: str = "dev-user"):
    ...
    access_token = create_access_token(data={"sub": user_id, "email": f"{user_id}@dev.local"})
