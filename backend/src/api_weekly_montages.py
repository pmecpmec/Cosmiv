...
def list_weekly_montages(
    ...
    with get_session() as session:
        ...
        montages = session.exec(query.order_by(WeeklyMontage.week_start.desc()).limit(limit)).all()
        ...
        for m in montages:
            ...
            render_paths = {}
            if m.job_id:
                renders = session.exec(select(Render).where(Render.job_id == m.job_id)).all()
                for r in renders:
...
def get_latest_montage():
    ...
    with get_session() as session:
        montage = session.exec(select(WeeklyMontage).order_by(WeeklyMontage.week_start.desc()).limit(1)).first()
        ...
        render_paths = {}
        if montage.job_id:
            renders = session.exec(select(Render).where(Render.job_id == montage.job_id)).all()
            for r in renders:
                render_paths[r.format] = r.output_path
...
def get_montage(montage_id: int):
    ...
    with get_session() as session:
        montage = session.exec(select(WeeklyMontage).where(WeeklyMontage.id == montage_id)).first()
