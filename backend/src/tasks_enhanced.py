...
def render_job_with_enhancements(job_id: str, target_duration: int):
    ...
    def run_stage(stage_name: str, percentage_start: int, percentage_end: int, 
                  message: str, func, critical: bool = False, *args, **kwargs):
        """Execute a pipeline stage with error handling"""
        update_progress(job_id, percentage_start, stage_name, message)
        try:
            result = func(*args, **kwargs)
            logger.info(f"Stage '{stage_name}' completed successfully",
                       extra={"job_id": job_id, "stage": stage_name})
            return result
        except RetryableException as e:
            add_error_detail(job_id, "WARNING", stage_name, f"Retryable error: {str(e)}")
            logger.warning(f"Retryable error in '{stage_name}': {str(e)}",
                          extra={"job_id": job_id, "stage": stage_name})
            if critical:
                raise  # Re-raise for retry
            return None  # Continue with degraded mode
        except Exception as e:
            add_error_detail(job_id, "CRITICAL" if critical else "WARNING", 
                           stage_name, str(e))
    ...
    try:
        ...
        run_stage("collect", 0, 5, "Collecting uploaded clips...", 
                 lambda: [video_files.extend([
        ...
        logger.info(f"Found {len(video_files)} video files", 
                   extra={"job_id": job_id, "stage": "collect"})
        ...
        run_stage("detect", 25, 50, "Detecting scenes and scoring highlights...",
                 lambda: [
                 ...
                 critical=False)  # Can continue even if some fail
        
        if not candidates:
            logger.warning("No scenes detected, using fallback",
                          extra={"job_id": job_id, "stage": "detect"})
            for vp in preprocessed:
        ...
        logger.info(f"Selected {len(selected)} scenes totaling {total:.1f}s",
                   extra={"job_id": job_id, "stage": "detect"})
        ...
        for v in variants:
            ...
            run_stage(
                f"render_{v}", render_pct - 15, render_pct,
                f"Rendering {v} format...",
                lambda fmt=v: video_outputs.update({fmt: render_with_fallback(
                    concat_path,
                    os.path.join(export_dir, f"video_{fmt}.mp4"),
                    preset=fmt
                )}),
                critical=True
            )
        ...
        music_result = run_stage(
            "music", 80, 85, "Generating music bed...",
        ...
        if preprocessed:
            transcript_result = run_stage(
                "stt", 85, 87, "Detecting profanity...",
                lambda: transcribe_audio(preprocessed[0]),
                critical=False
            )
        ...
        for v, vid in video_outputs.items():
            ...
            run_stage(
                f"mix_{v}", mix_pct - 4, mix_pct,
                f"Adding music and watermark to {v}...",
                lambda fmt=v, path=vid: _mix_final(fmt, path, music_path, mute_chain, 
                                                  export_dir, final_outputs),
                critical=True
            )
        ...
        public_map = {}
        run_stage(
            "upload", 95, 100, "Uploading final videos...",
        ...
        with get_session() as session:
            ...
            job.progress = json.dumps({"percentage": 100, "stage": "complete", 
        ...
        with get_session() as session:
            ...
            if job:
                ...
                session.add(job)
                session.commit()
        logger.error(f"Job {job_id} failed non-retryably: {str(e)}",
        ...
        with get_session() as session:
            ...
            if job:
                ...
                session.add(job)
                session.commit()
        logger.error(f"Job {job_id} failed with retryable error: {str(e)}",
                    extra={"job_id": job_id, "stage": "error"}, exc_info=True)
        raise RetryableException(f"Job failed: {str(e)}")


def _mix_final(format_name: str, video_path: str, music_path: str, mute_chain: str,
               export_dir: str, final_outputs: dict):
    ...
    cmd = [
        ...
        "-map", "[aout]",
        "-c:v", "copy" if vf_overlay == "null" else "libx264",
