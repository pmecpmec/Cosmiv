...
def render_job_with_enhancements(job_id: str, target_duration: int):
    ...
    def run_stage(
        ...
        **kwargs,
        ...
        try:
            ...
            add_error_detail(
                job_id, "WARNING", stage_name, f"Retryable error: {str(e)}"
            )
            logger.warning(
                f"Retryable error in '{stage_name}': {str(e)}",
                extra={"job_id": job_id, "stage": stage_name},
            )
            if critical:
            ...
            add_error_detail(
                job_id, "CRITICAL" if critical else "WARNING", stage_name, str(e)
            )
            logger.error(
    ...
    try:
        ...
        if not candidates:
            logger.warning(
                "No scenes detected, using fallback",
        ...
        logger.info(
            f"Selected {len(selected)} scenes totaling {total:.1f}s",
            extra={"job_id": job_id, "stage": "detect"},
        )
        ...
        if preprocessed:
            ...
            if transcript_result:
                ...
                mute_chain = build_profanity_mute_filters(
                    transcript.get("profanity", [])
                )
        ...
        for v, vid in video_outputs.items():
            mix_pct = 87 + (
                8 * (list(video_outputs.keys()).index(v) + 1) / len(video_outputs)
            )
            run_stage(
                f"mix_{v}",
                mix_pct - 4,
                ...
                lambda fmt=v, path=vid: _mix_final(
                    fmt, path, music_path, mute_chain, export_dir, final_outputs
                ),
                critical=True,
            )
        ...
        with get_session() as session:
            ...
            if job:
                ...
                session.commit()
        logger.error(
            f"Job {job_id} failed non-retryably: {str(e)}",
...
def _mix_final(
    ...
    inputs = ["-i", video_path]
    filter_complex = af + (
        ";[0:v]" + vf_overlay + "[vout]" if vf_overlay != "null" else ""
    )
