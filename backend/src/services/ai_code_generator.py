...
Generate the complete endpoint code."""
    ...
    def optimize_code(
        self, code: str, language: str, optimization_type: str = "general"
    ) -> Dict[str, Any]:
        """
...
Focus on: {optimization_type}"""
    ...
    def _build_context_prompt(self, context: Optional[Dict[str, Any]]) -> str:
        ...
        if context.get("existing_components"):
            context_parts.append(
                f"Existing components: {context['existing_components']}"
            )
    ...
    def get_generation(self, generation_id: str) -> Optional[CodeGeneration]:
        ...
        with get_session() as session:
            return session.exec(
                select(CodeGeneration).where(
                    CodeGeneration.generation_id == generation_id
                )
            ).first()
