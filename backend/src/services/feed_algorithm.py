...
class FeedAlgorithmService:
    ...
    def get_following_feed(user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        with get_session() as session:
            ...
            posts = session.exec(
                select(Post)
                .where(Post.user_id.in_(following_ids), Post.is_published == True)
    ...
    def get_for_you_feed(user_id: str, limit: int = 20) -> List[Dict[str, Any]]:
        ...
        with get_session() as session:
            ...
            if cache:
                ...
                # Fetch posts
                posts = session.exec(select(Post).where(Post.post_id.in_(post_ids), Post.is_published == True)).all()
            ...
            algo_state = session.exec(select(FeedAlgorithm).where(FeedAlgorithm.user_id == user_id)).first()

            # Get candidate posts (recent, published)
            time_threshold = datetime.utcnow() - timedelta(days=30)
            candidates = session.exec(
                select(Post).where(Post.is_published == True, Post.created_at > time_threshold)
            ).all()
            ...
            for post in candidates:
                ...
                total_score = engagement_score * 0.5 + recency_score * 0.3 + trending_score * 0.2
    ...
    def get_trending_feed(limit: int = 20) -> List[Dict[str, Any]]:
        ...
        with get_session() as session:
            ...
            posts = session.exec(select(Post).where(Post.is_published == True, Post.created_at > time_threshold)).all()
    ...
    def get_new_feed(limit: int = 20) -> List[Dict[str, Any]]:
        ...
        with get_session() as session:
            posts = session.exec(
                select(Post).where(Post.is_published == True).order_by(Post.created_at.desc()).limit(limit)
            ).all()
