...
from models import (
    ...
    StylePerformance, UserAnalytics, JobStatus
)
            ...
            if style_perf.avg_processing_time:
                ...
                style_perf.avg_processing_time = (
                    (style_perf.avg_processing_time * (style_perf.total_jobs - 1) + 
                     job.processing_time_seconds) / style_perf.total_jobs
                )
            else:
        ...
        if engagements:
            ...
            for e in engagements:
                if e.style_id:
                    ...
                    style_scores[e.style_id] += (
                        e.total_likes + e.total_shares + e.views
                    )
...
def get_time_series_metrics(
    ...
    with get_session() as session:
        ...
        if metric == "jobs":
            ...
            return sorted(
                [daily_stats[k] for k in daily_stats],
                key=lambda x: x["date"]
            )
