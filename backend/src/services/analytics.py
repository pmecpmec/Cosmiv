...
from models import Job, JobEngagement, SocialPostEngagement, SocialPost, StylePerformance, UserAnalytics, JobStatus
...
def track_job_view(job_id: str, user_id: Optional[str] = None) -> None:
    ...
    with get_session() as session:
        engagement = session.exec(select(JobEngagement).where(JobEngagement.job_id == job_id)).first()
...
def update_social_engagement(post_id: int, platform: str, metrics: Dict[str, Any]) -> None:
    """Update engagement metrics for a social media post"""
    with get_session() as session:
        engagement = session.exec(select(SocialPostEngagement).where(SocialPostEngagement.post_id == post_id)).first()
        ...
        if engagement.views > 0:
            engagement.engagement_rate = (
                (engagement.likes + engagement.shares + engagement.comments) / engagement.views
            ) * 100
...
def update_job_engagement_from_social(job_id: str) -> None:
    ...
    with get_session() as session:
        job_engagement = session.exec(select(JobEngagement).where(JobEngagement.job_id == job_id)).first()

        if not job_engagement:
            return

        # Get all social posts for this job
        posts = session.exec(select(SocialPost).where(SocialPost.job_id == job_id)).all()
        ...
        social_engagements = session.exec(
            select(SocialPostEngagement).where(SocialPostEngagement.post_id.in_(post_ids))
        ).all()
...
def update_style_performance(style_id: str, job_id: str) -> None:
    ...
    with get_session() as session:
        ...
        style_perf = session.exec(select(StylePerformance).where(StylePerformance.style_id == style_id)).first()
        ...
        if job.processing_time_seconds:
            if style_perf.avg_processing_time:
                ...
                style_perf.avg_processing_time = (
                    style_perf.avg_processing_time * (style_perf.total_jobs - 1) + job.processing_time_seconds
        ...
        if engagement:
            ...
            if style_perf.total_jobs > 0 and style_perf.total_views > 0:
                style_perf.avg_engagement_rate = (
                    (style_perf.total_likes + style_perf.total_shares) / style_perf.total_views
                ) * 100

        # Calculate performance score (0-100)
        # Score = weighted combination of success rate, engagement rate, and processing speed
        success_rate = (style_perf.successful_jobs / style_perf.total_jobs * 100) if style_perf.total_jobs > 0 else 0
...
def update_user_analytics(user_id: str) -> None:
    ...
    with get_session() as session:
        user_analytics = session.exec(select(UserAnalytics).where(UserAnalytics.user_id == user_id)).first()
        ...
        user_analytics.successful_jobs = sum(1 for j in jobs if j.status == JobStatus.SUCCESS)
        user_analytics.failed_jobs = sum(1 for j in jobs if j.status == JobStatus.FAILED)

        if user_analytics.total_jobs > 0:
            user_analytics.success_rate = user_analytics.successful_jobs / user_analytics.total_jobs * 100

        # Calculate avg processing time
        processing_times = [j.processing_time_seconds for j in jobs if j.processing_time_seconds]
        if processing_times:
            user_analytics.avg_processing_time = sum(processing_times) / len(processing_times)

        # Get engagement stats
        job_ids = [j.job_id for j in jobs]
        if job_ids:
            engagements = session.exec(select(JobEngagement).where(JobEngagement.job_id.in_(job_ids))).all()
            ...
            if user_analytics.total_views > 0:
                user_analytics.avg_engagement_rate = (
                    (user_analytics.total_likes + user_analytics.total_shares) / user_analytics.total_views
                ) * 100
        ...
        if engagements:
            ...
            if style_scores:
                user_analytics.best_performing_style = max(style_scores, key=style_scores.get)

        # Get social post stats
        posts = session.exec(select(SocialPost).where(SocialPost.user_id == user_id)).all()
        ...
        return [
            {
                ...
                "success_rate": (s.successful_jobs / s.total_jobs * 100) if s.total_jobs > 0 else 0,
...
def get_time_series_metrics(days: int = 7, metric: str = "jobs") -> List[Dict[str, Any]]:
    ...
    with get_session() as session:
        ...
        if metric == "jobs":
            ...
            return sorted([daily_stats[k] for k in daily_stats], key=lambda x: x["date"])

        elif metric == "engagement":
            # Group engagement by date
            engagements = session.exec(select(JobEngagement).where(JobEngagement.created_at >= start_date)).all()
