...
class MusicGenerator:
    ...
    @staticmethod
    def _build_prompt(style: Optional[str], tempo: Optional[str], mood: Optional[str]) -> str:
    ...
    def _generate_musicgen(duration: float, output_path: str, prompt: str) -> str:
        ...
        try:
            ...
            raise Exception("MusicGen dependencies not installed. Install: pip install transformers torch torchaudio")
        ...
        for i in range(num_chunks):
            audio_values = model.generate(**inputs, max_new_tokens=samples_per_chunk, do_sample=True)
    ...
    def _generate_mubert(duration: float, output_path: str, prompt: str) -> str:
        ...
        data = response.json()
        audio_url = data.get("audio_url") or data.get("url") or data.get("data", {}).get("url")
    ...
    def _generate_procedural(duration: float, output_path: str, prompt: str) -> str:
        ...
        is_energetic = "energetic" in prompt.lower() or "gaming" in prompt.lower() or "fast" in prompt.lower()
        ...
        for chunk_idx in range(num_chunks):
            ...
            subprocess.run([
                "ffmpeg", "-y", "-f", "lavfi",
                "-i", f"sine=frequency={chord_freq}:duration={chunk_dur}:sample_rate=44100",
            ...
            subprocess.run([
                "ffmpeg", "-y", "-f", "lavfi",
                "-i", f"sine=frequency={melodic_freq}:duration={chunk_dur}:sample_rate=44100",
                "-af", "volume=0.08,vibrato=f=4.5:d=0.3",
                temp2
            ], check=False)
            ...
            subprocess.run([
                "ffmpeg", "-y", "-f", "lavfi",
                "-i", f"sine=frequency={bass_freq}:duration={chunk_dur}:sample_rate=44100",
                "-af", "volume=0.15,lowpass=f=200",
                temp3
            ], check=False)
        ...
        for layer in [1, 2, 3]:
            ...
            with open(concat_list, "w") as f:
                for chunk_idx in range(num_chunks):
                    temp = os.path.join(temp_dir, f"aiditor_layer{layer}_{chunk_idx}.wav")
                    if os.path.exists(temp):
                        # Normalize path for concat file
                        normalized_path = temp.replace("\\", "/") if os.name == "nt" else temp
        ...
        if len(filter_parts) > 1:
            ...
            filter_complex_str = ";".join(filter_parts) + f";{mix_inputs}amix=inputs={len(filter_parts)}:duration=longest:dropout_transition=0[out]"
        else:
            filter_complex_str = filter_parts[0] + "[out]"
        
        # Add fade in/out and normalize
        filter_complex_str += ";[out]afade=t=in:st=0:d=1,afade=t=out:st={}:d=1,volume=0.2[final]".format(max(0, duration - 1))
        ...
        for layer in [1, 2, 3]:
            cleanup_files.append(os.path.join(temp_dir, f"aiditor_concat_layer{layer}.txt"))
...
def generate_music_bed(duration: float, output_path: str, freq: int = 220, style: Optional[str] = None) -> str:
    """
