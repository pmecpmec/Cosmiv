...
class MusicGenerator:
    ...
    def _generate_musicgen(duration: float, output_path: str, prompt: str) -> str:
        ...
        model = MusicgenForConditionalGeneration.from_pretrained(
            "facebook/musicgen-small", torch_dtype=torch.float16 if device == "cuda" else torch.float32
        )
        model.to(device)
    ...
    def _generate_procedural(duration: float, output_path: str, prompt: str) -> str:
        ...
        filter_complex_str += ";[out]afade=t=in:st=0:d=1,afade=t=out:st={}:d=1,volume=0.2[final]".format(
            max(0, duration - 1)
        )
